...
class ExploringBTCCloseClass:
    def btc_close_price(self,start_date,end_date):
        ...
        plt.title('BTC Close Price', fontsize=20, color='green')
        plt.xlabel("Close Dates", fontsize=17, color='blue')
        plt.ylabel("Close Price($)", fontsize=17, color='blue')
        fig = plt.gcf()
        fig.canvas.set_window_title('BTC-Close Price')
        plt.show()
