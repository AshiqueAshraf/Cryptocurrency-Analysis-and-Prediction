...
from PredictingETHOpen import PredictingETHOpenClass
from PredictingETHHigh import PredictingETHHighClass
from PredictingETHLow import  PredictingETHLowClass
from PredictingETHClose import PredictingETHCloseClass
from PredictingBTCVolume import PredictingBTCVolumeClass

class PredictingETHClass:
    ...
    def predict(self):
        ...
        if self.currency_s=="Open":
            print("Inside if...")
            ob = PredictingETHOpenClass()
            ob.predict_eth_open_price(res1)
        elif self.currency_s == "High":
            ob = PredictingETHHighClass()
            ob.predict_eth_high_price(res1)
        elif self.currency_s == "Low":
            ob = PredictingETHLowClass()
            ob.predict_eth_low_price(res1)
        elif self.currency_s == "Close":
            ob = PredictingETHCloseClass()
            ob.predict_eth_close_price(res1)
