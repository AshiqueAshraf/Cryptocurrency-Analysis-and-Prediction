...
from PredictionResult import PredictionResultClass

class PredictingDOGECloseClass:
    def predict_doge_close_price(self, pred_date):
        ...
        for k in data.index:
            if data.loc[k, "Close"] > 0.4:
                data.drop(k, inplace=True)

        for k in data.index:
            if data.loc[k, "Close"] < 0.005:
                data.drop(k, inplace=True)
        ...
        print("Close-r2 socre is ", score * 100)
        ...
        print(new_pred)
        print("---------------")
        price = float(new_pred)
        dats = str(pred_date[0][2]) + "-" + str(pred_date[0][1]) + "-" + str(pred_date[0][0])
        cry = "DOGE"
        stat = "Close"
...
#dt = [[2022, 12, 30]]
#ob.predict_doge_close_price(dt)
